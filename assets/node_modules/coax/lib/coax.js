/*
 * coax
 * https://github.com/jchris/coax
 *
 * Copyright (c) 2013 Chris Anderson
 * Licensed under the Apache license.
 */
var pax = require("pax"),
  hoax = require("hoax");

var coaxPax = pax();

coaxPax.extend("getQuery", function(params) {
  params = JSON.parse(JSON.stringify(params));
  var key, keys = ["key", "startkey", "endkey", "start_key", "end_key"];
  for (var i = 0; i < keys.length; i++) {
    key = keys[i];
    if (params[key]) {
      params[key] = JSON.stringify(params[key]);
    }
  }
  return params;
});

var Coax = module.exports = hoax.makeHoax(coaxPax());

Coax.extend("changes", function(opts, cb) {
  if (typeof opts === "function") {
    cb = opts;
    opts = {};
  }
  var self = this;
  opts = opts || {};
  opts.feed = "longpoll";
  opts.since = opts.since || 0;
  // console.log("change opts "+JSON.stringify(opts));
  self(["_changes", opts], function(err, ok) {
    if (err && err.code == "ETIMEDOUT") {
      self.changes(opts, cb); // TODO retry limit?
    } else if (err) {
      return cb(err);
    }
    ok.results.forEach(function(row){
      cb(null, row);
    });
    opts.since = ok.last_seq + 1;
    self.changes(opts, cb);
  });
});

//Coax.extend("forceSave", function(doc, cb) {
//	//console.log("forceSave method: doc "+JSON.stringify(doc));
//	console.log("forceSave method ");
//	var api = this(doc._id);
////	console.log("forceSave method: api.put ");
////	api.put(doc, cb);
//	console.log("forceSave method - get");
//	api.get(function(err, old) {
//		console.log("Is there any Error? " + err);
//		if (err && err.error !== "not_found") {
//			console.log("Some other type of error... " + err);
//			return cb(err);
//		}
//		if (!err) {
//			console.log("There was no error... " );
//			console.log("old._rev: " + old._rev);
//			doc._rev = old._rev;
//		} else {
//			console.log("Error: " + err.error);
//		}
//		console.log("forceSave method: put ");
//		api.put(doc, cb);
//	});
//});

Coax.extend("forceSave", function(doc, cb) {
	  var api = this(doc._id);
	  api.get(function(err, old) {
//		console.log("Is there any Error? " + err);
//		console.log("old: " + JSON.stringify(old));
//		console.log("old._headers: " + JSON.stringify(old._headers));
//		console.log("old.status: " + JSON.stringify(old.status));
//		console.log("old.upload: " + JSON.stringify(old.upload));
//		console.log("old.upload.status: " + JSON.stringify(old.upload.status));
	    
	    if (old.status != 404) {
	    	if (err && err.error !== "not_found") {
	  	      return cb(err);
	  	    }
	    }
	    if (!err) {
	      doc._rev = old._rev;
	    }
	    api.put(doc, cb);
	  });
	});
